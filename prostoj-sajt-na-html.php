...
<html lang="ru">
  <head>
    ...
    <title>Project Euler Blog: простой сайт на HTML</title>
  ...
  <body>
    ...
    <section>
      ...
      Весь процесс можно разбить на несколько простых шагов.</p>

      <h3>Размещение HTML-файла</h3>

      <p>По умолчанию XAMPP использует папку htdocs в качестве корневой директории сайта.
      Это значит, что все файлы для веб-доступа должны лежать именно там.</p>
      <p>На Ubuntu полный путь к этой папке обычно выглядит так:</p>
      <div class="vimcode">
        /opt/lampp/htdocs/
      </div>
    ...
    <section>
      <h3>Запуск веб-сервера</h3>

      <p>Самый простой способ управлять сервисами XAMPP — использовать его графическую панель управления.
      Однако для VPS, где графического интерфейса обычно нет, удобнее всего работать из командной строки.</p>

      <p>Основные команды для управления XAMPP:
        <ul>
          <li>Запустить все сервисы (Apache и MySQL):</li>
          <div class="vimcode">
            sudo /opt/lampp/lampp start
          </div>
          <li>Остановить все сервисы:</li>
          <div class="vimcode">
            sudo /opt/lampp/lampp stop
          </div>
          <li>Запустить только Apache (веб-сервер):</li>
          <div class="vimcode">
            sudo /opt/lampp/lampp startapache
          </div>
          <li>Перезапустить все сервисы (например, после изменения настроек):</li>
          <div class="vimcode">
    ...
    <section>
      ...
      <p>После того как сервер запущен, а порт открыт, открыть браузер
      и ввести в адресной строке публичный IP-адрес VPS-сервера.</p>

      <div class="vimcode">
        http://&lt;IP-адрес&gt;
      </div>

      <p>Если файлы находятся в папке с именем <b>мой-сайт</b>, то адрес будет выглядеть так:</p>
      <div class="vimcode">
        http://&lt;IP-адрес&gt;/мой-сайт/
      </div>

      <p>На экране должна отобразиться HTML-страница сайта.</p>
    </section>

    <section>
      <h2>Пример работы с простым сайтом на HTML</h2>
      
      <p>Примером простого сайта на HTML является проект
      <a href="https://github.com/AlexandrAnatoliev/project-euler-blog">Project-Euler-Blog</a>
      - сборник статей для моего блога.<br>
      Я использую его для более легкой верстки, т.к. HTML файлы можно смотреть просто в браузере,
      не добавляя в базу данных и не запуская XAMPP.</p>
    </section>
    ...
    <section>
      ...
      <p>Поэтому при клонировании репозитория использовать флаг <b>--recursive</b></p>
      <div class="vimcode">
        git clone --recursive https://github.com/AlexandrAnatoliev/project-euler-blog.git
      </div>
        
      <p>Иначе при обычном клонировании папка src/styles будет пустой:</p>
<pre class="vimcode">
...
git submodule update
    ...
    <section>
      <h3>Обновление CSS-подмодуля до последней версии</h3>

      <p>Перейти в папку подмодуля:</p>
      <div class="vimcode">
        cd src/styles
      </div>

      <p>Обновиться до последнего коммита в ветке main:</p>
<pre class="vimcode">
git checkout main
git pull
</pre>

      <p>Вернуться в основной репозиторий:</p>
      <div class="vimcode">
        cd ../..
      </div>

      <p>Зафиксировать изменение (новую ссылку на коммит):</p>
<pre class="vimcode">
git add src/styles
git commit -m "Update css to latest version"
    ...
    <section>
      ...
      в этот проект (
      <a href="https://github.com/AlexandrAnatoliev/project-euler-blog">Project-Euler-Blog</a>).</p>
      ...
      Откроется маленькое окно. Скопируйте из него URL и выполните на своем компьютере команду:</p>
      <div class="vimcode">
      ...
      <p>4. Добавьте ссылку на оригинальный репозиторий для будущих обновлений:</p>
      <div class="vimcode">
      ...
      <p>Вы должны увидеть origin (создается автоматически при клонировании) и upstream-ремоуты:</p>

<pre class="vimcode">
origin  https://github.com/&lt;your-username&gt;/project-euler-blog.git (fetch)
origin  https://github.com/&lt;your-username&gt/project-euler-blog.git (push)
...
upstream        https://github.com/AlexandrAnatoliev/project-euler-blog.git (push)
    ...
    <section>
      ...
      <p>3. Отправьте изменения в ваш "форкнутый" репозиторий:</p>
      <div class="vimcode">
      ...
      <ul>
        ...
        <li>
          ...
          request оригинального репозитория проекта project-euler-blog.
        </li>
        <li>
    ...
    <section>
      ...
      <p>Столкнувшись с затруднениями, не стесняйтесь открыть <b>issue</b>, написать в
      <a href="https://github.com/AlexandrAnatoliev/project-euler-blog/discussions/14"><b>Discussions</b></a>
      или мне на почту <b>per-1986@list.ru</b>.</p>
    </section>
